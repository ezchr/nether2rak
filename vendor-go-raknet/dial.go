...
//
// PATCHED (not upstream): Dragonfly's own go-raknet fork
// (github.com/sandertv/go-raknet, pinned via replace in the dragonfly
// project's go.mod) added a check in handler.go's handleOpenConnectionRequest2:
// "Vanilla clients always provide a negative ClientGUID" - it hard-rejects any
// ClientGUID >= 0 with "invalid ClientGUID '%d', expected negative" and blocks
// the source address for 10s. This relay's ClientGUID (both the ping above and
// connState.id below) must therefore always be negative, matching what a real
// Bedrock client sends - confirmed live 2026-09-17 against a Dragonfly server
// running that fork: an unpatched positive GUID caused every relay join
// attempt to fail with "i/o timeout" (the relay's dial being silently dropped
// by Dragonfly's 10s block), while direct-IP joins from a real client worked
// fine. See connState.id's assignment in DialContext for the second site.
...
func (dialer Dialer) DialContext(ctx context.Context, address string) (*Conn, error) {
	...
	cs := &connState{
		...
		id:                 -atomic.AddInt64(&dialerID, 1), // PATCHED: negated, see dialerID comment above
