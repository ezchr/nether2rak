...
## Reaching the backend over NetherNet instead of RakNet

By default this relay speaks RakNet to the backend (Geyser, matching the setup steps above). Set
`backend_transport` to `"nethernet"` in `config.json` to reach a backend that speaks NetherNet
instead — a Dragonfly server with its own NetherNet listener enabled, for example. This works
end-to-end now: login, resource packs, and spawn all complete correctly against a NetherNet
backend, confirmed live against Dragonfly.

When using NetherNet, `nethernet_backend_address` should point at the backend's NetherNet HTTP
signaling endpoint — for Dragonfly this is the same host:port its regular listener binds to, not
a separate port. The client-facing side of the relay is unaffected either way; this setting only
changes how the relay itself reaches whatever's behind it.
