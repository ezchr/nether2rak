...
func (conn *Conn) addRemoteCandidate(candidate webrtc.ICECandidate) error {
	if err := conn.ice.AddRemoteCandidate(&candidate); err != nil {
		conn.log.Debug("DEBUGPATCH: AddRemoteCandidate failed", "candidate", candidate.String(), "err", err)
		return fmt.Errorf("add remote candidate: %w", err)
	}
	conn.log.Debug("DEBUGPATCH: AddRemoteCandidate succeeded", "candidate", candidate.String(), "typ", candidate.Typ.String())
...
func (conn *Conn) trickleCandidates(signaling Signaling) error {
	...
	conn.log.Debug("DEBUGPATCH: trickleCandidates starting, registering OnLocalCandidate and calling Gather()")
	conn.gatherer.OnLocalCandidate(func(candidate *webrtc.ICECandidate) {
		...
		conn.log.Debug("DEBUGPATCH: local candidate found", "candidate", candidate.String(), "typ", candidate.Typ.String())
	...
	err := conn.gatherer.Gather()
	conn.log.Debug("DEBUGPATCH: Gather() returned", "err", err)
	return err
}
...
func (desc description) encode() ([]byte, error) {
	...
	for i, candidate := range desc.candidates {
		// EXPERIMENT (2026-08-17): advertise only IPv4 host candidates.
		//
		// pion emits two things a real Bedrock dedicated server never does:
		// an IPv6 host candidate (when the box has IPv6), and srflx/relay
		// candidates carrying "raddr 0.0.0.0" - semantically invalid, since the
		// related address should be the local base address, but pion binds a
		// wildcard socket so it has none to report. A strict SDP/candidate parser
		// on the client could discard the whole candidate list over either one.
		//
		// This host is directly reachable on arbitrary UDP ports (verified), so a
		// lone IPv4 host candidate is sufficient and is what a dedicated server
		// with a public address would realistically advertise.
		...
		media.Attributes = append(media.Attributes, sdp.Attribute{
			...
			Value: strings.TrimPrefix(formatICECandidate(i, candidate, desc.ice), "candidate:"),
		})
	}
